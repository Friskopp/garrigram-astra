...
## Optional photo and comment notifications

Edit profile → Notifications → **New photos on this device** is off by default. Enabling it requests browser permission, stores that device’s Web Push subscription under the verified account, and reveals **Send test notification**. **Comments on my posts** is a separate opt-in toggle on the same device. Either type can be on independently. Turning a type off immediately stops delivery for that type; the browser subscription is retained so switching back on does not need a new permission prompt. Other devices are unchanged. On iPhone/iPad, add Garrigram to the Home Screen and open it there first. A manifest and GA app icons support installation. The local Node demo displays the control but does not send push messages.
...
Comment notifications use `push_comment_events`, separate per-device preferences/cursors, and the existing queue. Only another account's new comment on your own non-demo post is eligible. Deleted posts/comments and your own comments are excluded; editing a comment does not create an alert. Existing photo subscribers retain their photo setting and start with comment alerts off. Enabling either topic starts at the current event cursor, so activity from before opt-in or while disabled is not replayed. Comment alerts use the same two-minute grouping window and their own notification tag, and open the most recently commented post in the group. No comment text or author identity appears on the lock screen.
