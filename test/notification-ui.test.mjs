...
async function ui({permission='default',ios=false,installed=false,available=true,active=false,failDelete=false}={}){
 const makeToggle=()=>({disabled:true,setAttribute(k,v){this[k]=v;}});const toggle=makeToggle(),commentToggle=makeToggle(),help={},testButton={},handlers={};
 const section={querySelector(s){return s==='#photo-notifications'?toggle:s==='#comment-notifications'?commentToggle:s==='#notification-help'?help:testButton;}};
 ...
 let server={photos:active,comments:false},registered=active,sub=active?subscription():null,prompts=0,subscriptions=0,unsubscriptions=0;
 function subscription(){return {endpoint:'https://web.push.apple.com/test',toJSON(){return {endpoint:this.endpoint,keys:{p256dh:'test',auth:'test'}};},async unsubscribe(){unsubscriptions++;sub=null;return true;}};}
 const registration={pushManager:{async getSubscription(){return sub;},async subscribe(){subscriptions++;sub=subscription();return sub;}}};
 ...
 const controls=context.initNotifications({api:async(path,options)=>{
  ...
  if(path.endsWith('/status'))return {registered,enabled:server.photos||server.comments,preferences:{...server}};
  if(['PUT','PATCH'].includes(options.method)){const data=JSON.parse(options.body);if(failDelete&&Object.values(data.preferences).includes(false))throw new Error('Offline');server={...server,...data.preferences};registered=true;}
  return {enabled:server.photos||server.comments,preferences:{...server}};
 }});
 return {toggle,commentToggle,help,testButton,controls,get prompts(){return prompts;},get server(){return server.photos;},get preferences(){return server;},get subscriptions(){return subscriptions;},get unsubscriptions(){return unsubscriptions;}};
}
...
test('service worker shows alerts without network access and constrains notification navigation to Garrigram',async()=>{
 ...
 events.push({data:{json:()=>({body:'Someone commented on your post.',url:'/?post=abc#feed',tag:'garrigram-comments'})},waitUntil(p){pending=p;}});await pending;assert.equal(notifications[1].tag,'garrigram-comments');
...
test('comment toggle works independently, reuses phone permission/subscription, and persists after reopening',async()=>{
 const f=await ui();await f.controls.refresh();await f.commentToggle.onclick();
 assert.deepEqual(f.preferences,{photos:false,comments:true});assert.equal(f.prompts,1);assert.equal(f.subscriptions,1);
 await f.toggle.onclick();assert.deepEqual(f.preferences,{photos:true,comments:true});assert.equal(f.prompts,1);assert.equal(f.subscriptions,1);
 await f.commentToggle.onclick();assert.deepEqual(f.preferences,{photos:true,comments:false});assert.equal(f.unsubscriptions,0);assert.equal(f.testButton.hidden,false);
 await f.controls.refresh();assert.equal(f.commentToggle['aria-checked'],'false');assert.equal(f.toggle['aria-checked'],'true');
 await f.toggle.onclick();assert.equal(f.testButton.hidden,true);await f.commentToggle.onclick();assert.equal(f.subscriptions,1);assert.equal(f.prompts,1);
});
