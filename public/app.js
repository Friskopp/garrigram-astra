...
import { initPeople, avatarMarkup } from './people.mjs';
let peopleUI;
import { effectSvg } from './effects.mjs';
import { mapPosts, createHeatLayer } from './heatmap.mjs';
let heatLayer, mapMode = 'photos', feedOrder = 'latest', detailPostId = null;
...
icons.back='<path d="m15 5-7 7 7 7"/>';
...
function renderName(){$('profile-initials').textContent=displayName?initials(displayName):'Y';$('profile-name').textContent=displayName?displayName.split(' ')[0]:'Your name';peopleUI?.header();}
renderName();
...
function renderFeed(){
  const query=$('feed-search').value.trim().toLocaleLowerCase();
  const visible=posts.filter(post=>[post.author,post.caption,post.location].some(value=>String(value||'').toLocaleLowerCase().includes(query)));
  visible.sort((a,b)=>Number(a.demo)-Number(b.demo)||(feedOrder==='popular'?(b.likes||0)-(a.likes||0):0)||Date.parse(b.created_at)-Date.parse(a.created_at));
  const count=location.hash==='#map'?posts.length:visible.length;$('moment-count').textContent=`${count} moment${count===1?'':'s'}`;
  $('sample-note').hidden=!posts.some(p=>p.demo);
  if(visible.length)$('feed').innerHTML=visible.map(post=>card(post)).join('');
  else if(query){$('feed').innerHTML='<div class="empty-state"><h3>No moments found.</h3><p>Try another name, caption or place.</p><button class="text-button" id="clear-search">Clear search</button></div>';$('clear-search').onclick=()=>{$('feed-search').value='';renderFeed();$('feed-search').focus();};}
  else{$('feed').innerHTML=`<div class="empty-state">${icon('camera')}<h3>Your everyday belongs here.</h3><p>Share a photo from the office, a coffee break,<br>or wherever the day takes you.</p><button class="primary" id="empty-share">Share the first moment</button></div>`;$('empty-share').onclick=openCompose;}
}
$('feed-search').oninput=renderFeed;
for(const order of ['latest','popular'])$('sort-'+order).onclick=()=>{feedOrder=order;for(const choice of ['latest','popular'])$('sort-'+choice).setAttribute('aria-pressed',String(choice===order));renderFeed();};
function renderPostDetail(){const post=posts.find(post=>post.id===detailPostId);if(post)$('full-photo-view').innerHTML=card(post,{detail:true});}
async function openPostDetail(post){
  detailPostId=post.id;commentState(post.id).open=true;renderPostDetail();
...
async function postAction(e){const button=e.target.closest('[data-action]');if(!button)return;const article=button.closest('[data-post]');const post=posts.find(p=>p.id===article?.dataset.post);if(!post)return;
  ...
  if(button.dataset.action==='map'){$('photo-dialog').close();$('map-period').value='all';setMapMode('photos');location.hash='map';showView();map?.setView([post.lat,post.lng],15);showMapDetail(post);}
}
$('feed').addEventListener('click',postAction);$('map-detail').addEventListener('click',postAction);$('full-photo-view').addEventListener('click',postAction);
...
function commentsMarkup(post){
  ...
  return `<section class="comments" aria-label="Comments on moment by ${escapeHTML(post.author)}"><h3 class="comments-title">Comments</h3>
...
for(const parent of [$('feed'),$('map-detail'),$('full-photo-view')]){
...
function renderMarkers(){if(mapMode==='people'){if(heatLayer&&map.hasLayer(heatLayer))map.removeLayer(heatLayer);renderMarkerPins();$('map-summary').hidden=true;peopleUI?.renderMap();return;}const located=mapPosts(posts,$('map-period').value);
  if(mapMode==='heat'){
  ...
  const placesByName=new Map();for(const post of shown){const name=(post.location||'Pinned location').split(' · ')[0];placesByName.set(name,(placesByName.get(name)||0)+1);}
...
function fitMap(){if(!map)return;if(mapMode==='people'){peopleUI?.fit();return;}const located=mapPosts(posts,$('map-period').value).filter(post=>mapMode!=='heat'||!post.demo);if(located.length)map.fitBounds(L.latLngBounds(located.map(post=>[post.lat,post.lng])).pad(.25),{maxZoom:15});}
function setMapMode(mode){
  mapMode=mode;
  $('map-photos').setAttribute('aria-pressed',String(mode==='photos'));
  $('map-heat').setAttribute('aria-pressed',String(mode==='heat'));
  $('map-people').setAttribute('aria-pressed',String(mode==='people'));
  document.querySelector('.map-period').hidden=mode==='people';
  $('heat-legend').hidden=mode!=='heat';
  $('map-detail').innerHTML='';
  if(map)renderMarkers();
  peopleUI?.modeChanged();
}
$('map-photos').onclick=()=>setMapMode('photos');
$('map-heat').onclick=()=>setMapMode('heat');
$('map-people').onclick=()=>setMapMode('people');
...
function showView(){const isMap=location.hash==='#map';document.querySelector('.app-layout').classList.toggle('map-layout',isMap);$('feed-view').hidden=isMap;$('map-view').hidden=!isMap;renderFeed();$('view-title').innerHTML=isMap?'The map<span class="orange-dot">.</span>':'Real moments.<br><span class="mint-text">Real people.</span>';$('view-eyebrow').textContent=isMap?'HERE, THERE & EVERYWHERE':'LIFE, LATELY';for(const [id,active]of[['feed-tab',!isMap],['map-tab',isMap],['mobile-feed',!isMap],['mobile-map',isMap]]){$(id).classList.toggle('active',active);if(active)$(id).setAttribute('aria-current','page');else $(id).removeAttribute('aria-current');}if(isMap){initMap();requestAnimationFrame(()=>map?.invalidateSize({pan:false}));}}
...
peopleUI=initPeople({api,toast,getMap:()=>map,isPeople:()=>mapMode==='people',getName:()=>displayName,onProfile:async profile=>{saveName(profile.name);await loadPosts();}});
initNotifications({api});
await peopleUI.start();
try{const session=await api('/api/session');if(session.hosted){$('profile-description').textContent='Your profile is shared across devices, posts, comments and the People map.';document.querySelector('.local-badge').textContent='GARRISON ONLY';const info=document.createElement('p');info.className='file-help';info.textContent='Signed in as '+session.email;$('profile-form').append(info);const logout=document.createElement('a');logout.href='/cdn-cgi/access/logout';logout.className='text-button';logout.textContent='Sign out';$('profile-form').append(logout);}}catch{}
await loadPosts();showView();
const linkedPost=new URL(location.href).searchParams.get('post');
if(linkedPost){const post=posts.find(p=>p.id===linkedPost);if(post)await openPostDetail(post);else toast('This moment is no longer available.');const clean=new URL(location.href);clean.searchParams.delete('post');history.replaceState(null,'',clean);}
